...
router.post("/login", loginLimiter, validate(loginSchema), asyncHandler(async (req, res) => {
  ...
  res.json({
    token,
    user: { id: user.id, name: user.name, email: user.email, role: user.role, branch: user.branch, designation: user.designation || "", agency_id: user.agency_id, avatar_url: user.avatar_url || null, phone: user.phone || null, created_at: user.created_at }
  });
}));
